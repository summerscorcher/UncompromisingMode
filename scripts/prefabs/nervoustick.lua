...
local function DespawnTick(inst)
...
local function fn()
    ...
    inst:WatchWorldState("isday", DespawnTick)
    inst:WatchWorldState("iscaveday", DespawnTick)
...
local function OnKilledDen(inst)
...
local function denfn()
    ...
    inst:AddComponent("combat")
    inst:ListenForEvent("death", OnKilledDen)

    inst.beattask = inst:DoTaskInTime(.75 + math.random() * .75, beat)

    inst:WatchWorldState("isday", Despawn)
    inst:WatchWorldState("iscaveday", Despawn)
